...
const ScreenDrawer = (props: ScreenDrawerProps) => {
  ...
  return (
    ...
    >
      ...
      >
        {hasNotchIcon && (
          <View style={[styles.topNotch, { backgroundColor: theme.colors.dividerLong }]} />
        )}
        ...
        <Layout hasPadding={false}>
          ...
          >
            {hasBack && (
              <TouchableOpacity
                onPress={() =>
                  Platform.OS === 'web' ? window.history.go(-1) : navigation.goBack()
                }
                {...testProps(`${screenName.replace(' ', '')}_btn_navigate_back`)}
