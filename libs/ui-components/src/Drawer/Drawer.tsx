...
const Drawer = (props: DrawerProps) => {
  ...
  useEffect(() => {
    const keyboardDidShow = Keyboard?.addListener('keyboardDidShow', listenerShow);
    const keyboardDidHide = Keyboard?.addListener('keyboardDidHide', listenerRemove);
    return () => {
  ...
  return (
    <Modal visible={showDrawer} transparent onRequestClose={() => setShowDrawer(false)}>
      <View style={{ flexDirection: 'row', height: '100%', width: '100%' }}>
        {isDesktop && <View style={{ maxWidth: 475, width: '100%' }} />}

        <View style={[styles.wrapper, { height: screenHeight, flex: 1 }]}>
          {showBackground}
          <TouchableOpacity onPress={() => setShowDrawer(false)} style={{ flex: 1 }} />
            ...
            <View style={{ width: '100%', alignItems: 'center' }}>
              ...
              >
                ...
                {hasBack && (
                  ...
                  >
                    <Icon name="LeftIcon" color={theme.colors.textPrimary} width={20} height={20} />
                    <Text
                      variant="heading2"
