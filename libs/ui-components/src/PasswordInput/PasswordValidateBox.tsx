...
const PasswordValidateBox = ({ errorLabel, data, isFocused }: PasswordValidateBoxProps) => {
  const { theme } = useContext(ThemeProvider);
  return (
    <View style={[styles.validatorBox, { color: theme.colors.backgroundSurface2 }]}>
...
const RenderIcon = (props: any) => {
  ...
  if (!data?.checkStatus && !isFocused) {
    return (
      ...
      >
        <Icon name="CloseIcon" color={theme.colors.backgroundSurface1} width={5.31} height={5.07} />
      </View>
    );
  }
  if (data?.checkStatus) {
    return (
      <View style={[styles.validateTickContainer, { backgroundColor: theme.colors.secondary1 }]}>
        <Icon name="TickIcon" color={theme.colors.backgroundSurface1} width={6.76} height={5.07} />
      </View>
    );
  }
  return <View style={[styles.inValidateTickContainer, { borderColor: theme.colors.border }]} />;
};
